...
# https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-hadoop-task-config.html#emr-hadoop-task-config-m1
...
spark = SparkSession.builder \
    .appName('airflow-weather-project-emr-raw-to-clean-hourly') \
...
# Need to have Livy installed to access glue catalog dbs; also upgrade to latest version of emr 6.9.0
# Need to have "Use for Hive and Spark table metadata" checked - especially in old EMR UI
spark.catalog.setCurrentDatabase('airflow-weather-project-clean')
df = spark.sql('select case when max(current_dt) is null then 0 else max(current_dt) end as max_current_dt from airflow_weather_project_clean_hourly_weather')
...
df_spark_2 = spark.sql(f'''select * from raw_weather_data where current.dt > ({latest_processed_data_date} - 86400 * 3)''')

if df_spark_2.take(1):
    # Step 3: Get hourly weather into separate dataframe
    df_hourly_3 = df_spark_2.select('lat', 'lon', 'timezone', 'current.dt', 'timezone_offset', 'hourly')

    # Step 4: Expand hourly array
    df_hourly_array_expand_4 = df_hourly_3.select('*', explode(col('hourly'))) \
        .drop('hourly') \
        .withColumnRenamed('dt', 'current_dt') \
        .withColumnRenamed('col', 'hourly_weather_forecast')

    # Step 5: Flatten hourly structs
    df_hour_flatten_5 = flatten_struct_prefix(df_hourly_array_expand_4, 'hourly_weather_forecast')

    struct_cols_to_flatten = []
    iterations = 1
    df_temp_1 = df_hour_flatten_5
    df_temp_1.cache()
    ...
    df_current_flatten_6 = df_temp_1

    # Step 6: Coalesce duplicate cols
    ...
    iterations = 1
    df_temp_1 = df_current_flatten_6
    df_temp_1.cache()
    ...
    df_hourly_coalesce_7 = df_temp_1

    # Step 7: Expand hourly forecast weather array
    df_hour_expand_weather_8 = split_array_prefix(df_hourly_coalesce_7, 'hourly_weather_forecast_weather')
    df_hour_expand_weather_9 = flatten_struct_prefix(df_hour_expand_weather_8, 'hourly_weather_forecast_weather[0]')

    # Step 8: Add date and timestamp cols
    df_final_10 = df_hour_expand_weather_9 \
    ...
    d_schema = {
        ...
        'current_dt': 'integer',
        'timezone_offset': 'integer',
        'hourly_weather_forecast_dt': 'integer',
        'hourly_weather_forecast_feels_like': 'integer',
        'hourly_weather_forecast_pressure': 'integer',
        'hourly_weather_forecast_humidity': 'integer',
        'hourly_weather_forecast_clouds': 'integer',
        'hourly_weather_forecast_visibility': 'integer',
        'hourly_weather_forecast_wind_deg': 'integer',
        'hourly_weather_forecast_wind_speed': 'double',
        'hourly_weather_forecast_dew_point': 'double',
        'hourly_weather_forecast_wind_gust': 'double',
        'hourly_weather_forecast_temp': 'double',
        'hourly_weather_forecast_uvi': 'double',
        'hourly_weather_forecast_pop': 'double',
        'hourly_weather_forecast_weather[0]_id': 'integer',
        'hourly_weather_forecast_weather[0]_main': 'string',
        'hourly_weather_forecast_weather[0]_description': 'string',
        'hourly_weather_forecast_weather[0]_icon': 'string',
        'hourly_weather_forecast_rain_1h': 'double',
    ...
    df_final_11.write.mode('overwrite').partitionBy('current_weather_date_partition') \
        .parquet('/user/hadoop/processed/clean/hourly')