...
df = spark.sql('''select case when max(current_weather_time) is null then cast('1970-01-01' as timestamp) else max(current_weather_time) end as max_current_weather_time from airflow_weather_project_analytic_hourly_weather''')
...
df_spark_2 = spark.read.parquet('/user/hadoop/processed/clean/hourly')
df_spark_2.createOrReplaceTempView('airflow_weather_project_clean_hourly_weather')
df_spark_3 = spark.sql(f'''select * from airflow_weather_project_clean_hourly_weather where current_weather_time > cast('{latest_processed_data_date}' as string)''')

if df_spark_3.take(1):
    # Step 3: Adjust cols
    df_rename_3 = df_spark_3 \
        .withColumnRenamed('hourly_weather_forecast_weather[0]_main', 'hourly_weather_state') \
        .withColumnRenamed('hourly_weather_forecast_weather[0]_description', 'hourly_weather_state_desc') \
        .drop('hourly_weather_forecast_weather[0]_id', 'hourly_weather_forecast_weather[0]_icon', 'current_dt')
    ...
    # df_final_5 = df_reorder_4 \
    #     .withColumn('hourly_weather_forecast_feels_like', col('hourly_weather_forecast_feels_like').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_wind_speed', col('hourly_weather_forecast_wind_speed').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_dew_point', col('hourly_weather_forecast_dew_point').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_uvi', col('hourly_weather_forecast_uvi').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_temp', col('hourly_weather_forecast_temp').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_wind_gust', col('hourly_weather_forecast_wind_gust').cast(DoubleType())) \
    #     .withColumn('hourly_weather_forecast_pop', col('hourly_weather_forecast_pop').cast(DoubleType()))
    ...
    df_reorder_4.write.mode('overwrite').partitionBy('current_weather_date_partition') \
        .parquet('/user/hadoop/processed/analytic/hourly')